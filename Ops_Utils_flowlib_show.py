...
from Ops_Utils_pfm import *


__all__ = [
    'visualize_flow', 'flow_to_image', 'warp_image'
]
...
def evaluate_flow_file(gt, pred):
    ...
    # Calculate errors
    average_pe = flow_error(gt_flow[:, :, 0], gt_flow[:, :, 1], eva_flow[
                            :, :, 0], eva_flow[:, :, 1])
    return average_pe
...
def evaluate_flow(gt_flow, pred_flow):
    ...
    pred: estimated flow
    """
    average_pe = flow_error(gt_flow[:, :, 0], gt_flow[:, :, 1], pred_flow[
                            :, :, 0], pred_flow[:, :, 1])
    return average_pe
...
def warp_image(im, flow, **kwargs):
    ...
    (iy, ix) = np.mgrid[0:image_height:1., 0:image_width:1.]
    (fy, fx) = np.mgrid[0:flow_height:1., 0:flow_width:1.]
    fx -= flow[:, :, 0]
    fy -= flow[:, :, 1]
    mask = np.where(fx < 0) or np.where(fx > flow_width) \
        or np.where(fy < 0) or np.where(fy > flow_height)
    ...
    for i in range(im.shape[2]):
        ...
        new_channel = scipy.interpolate.griddata(points, values, xi, **kwargs)
        warp[:, :, i] = new_channel.reshape(image_height, image_width)
        warp[mask] = np.nan
    return warp
...
def compute_color(u, v):
    ...
    # LINE CHANGED FOR COHERANCE WITH MIDDLEBURY
    # ALTHOUGH THERE MAY BE A MISTAKE
    # OLD LINE:
...
show_flow('00017.flo')
#print(read_flow("00001.flo").shape)
